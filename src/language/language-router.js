...
languageRouter.post("/guess", jsonBodyParser, async (req, res, next) => {
  try {
    ...
    const head = await LanguageService.getWordById(
      req.app.get("db"),
      language.head
    );
    ...
    const traversetoMemoryValue = await getNthMemory(
      req.app.get("db"),
      head,
      head.memory_value
    );
    ...
    LanguageService.updateWordPointer(req.app.get("db"), head).then(() =>
      next()
    );
    LanguageService.updateWordPointer(
      req.app.get("db"),
      traversetoMemoryValue
    ).then(() => next());
    LanguageService.updateHead(req.app.get("db"), language).then(() => next());
...
async function getNthMemory(db, node, nth, result = 0) {
  if (node.next === null || nth === result) return node;
